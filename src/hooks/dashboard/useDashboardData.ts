...
export const useDashboardData = (): UseDashboardDataReturn => {
  ...
  const [summaryChartData, setSummaryChartData] = useState<SummaryChartResponse[]>([]);
  const [skuTypeRatioData, setSkuTypeRatioData] = useState<SkuTypeRatioChartResponse[]>([]);
  const [optimizationIndexData, setOptimizationIndexData] = useState<OptimizationIndexResponse[]>([]);
  const [storageStatusData, setStorageStatusData] = useState<StorageStatusResponse | null>(null);
  ...
  const [isLoadingOptimizationIndex, setIsLoadingOptimizationIndex] = useState(false);
  ...
  const [summaryChartError, setSummaryChartError] = useState<string | null>(null);
  const [skuTypeRatioError, setSkuTypeRatioError] = useState<string | null>(null);
  const [optimizationIndexError, setOptimizationIndexError] = useState<string | null>(null);
  ...
  const fetchImportChart = useCallback(async (request: ImportChartRequest) => {
    ...
    try {
      ...
      setImportChartError(error instanceof Error ? error.message : "Failed to fetch import chart data");
    } finally {
  ...
  const fetchExportChart = useCallback(async (request: ExportChartRequest) => {
    ...
    try {
      ...
      setExportChartError(error instanceof Error ? error.message : "Failed to fetch export chart data");
    } finally {
      setIsLoadingExportChart(false);
    }
  }, []);

  const fetchSummaryChart = useCallback(async (request: SummaryChartRequest) => {
    ...
    try {
      ...
      setSummaryChartError(error instanceof Error ? error.message : "Failed to fetch summary chart data");
    } finally {
  ...
  const fetchSkuTypeRatio = useCallback(async () => {
    ...
    try {
      ...
      setSkuTypeRatioError(error instanceof Error ? error.message : "Failed to fetch SKU type ratio data");
    } finally {
      setIsLoadingSkuTypeRatio(false);
    }
  }, []);

  const fetchOptimizationIndex = useCallback(async (request: OptimizationIndexRequest) => {
    ...
    try {
      ...
      setOptimizationIndexError(error instanceof Error ? error.message : "Failed to fetch optimization index data");
    } finally {
      setIsLoadingOptimizationIndex(false);
    }
  }, []);

  const fetchStorageStatus = useCallback(async (request: StorageStatusRequest) => {
    ...
    try {
      ...
      setStorageStatusError(error instanceof Error ? error.message : "Failed to fetch storage status data");
    } finally {
  ...
  // PDF export function
  const exportWarehousePdf = useCallback(async (request: StorageStatusRequest) => {
    try {
      ...
      link.href = url;
      link.download = `warehouse-summary-${new Date().toISOString().split('T')[0]}.pdf`;
